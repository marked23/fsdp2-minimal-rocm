...
def main(args):
    ...
    # After fully_shard, the model is transformed into an FSDP module while maintaining
    # its original functionality. The type system doesn't understand this transformation
    # well, so we need to ignore type checking for FSDP-related operations.
    ...
    if args.mixed_precision:
        inspect_mixed_precision(model)  # type: ignore

    optim = torch.optim.Adam(model.parameters(), lr=1e-2)
    if checkpointer.last_training_time is not None:
        checkpointer.load_optim(model, optim)  # type: ignore
    ...
    checkpointer.save(model, optim) # type: ignore
